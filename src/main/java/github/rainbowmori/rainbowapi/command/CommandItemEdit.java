		...
		new CommandTree("ie")
			...
			.then(new LiteralArgument("lore")
				.then(new LiteralArgument("add")
					.then(new GreedyStringArgument("name")
						.executesPlayer((sender, args) -> {
							itemEdit(sender, builder -> builder.addLore(Util.mm(args[0])));
						})))
				.then(new LiteralArgument("remove")
					.then(new IntegerArgument("line", 1)
						.executesPlayer((sender, args) -> {
							itemEdit(sender, builder -> {
								int arg = (int) args[0];
								if (builder.getLore().size() < arg) {
									sender.sendMessage(Util.mm("<blue>" + builder.getLore().size() + "<red>列しかアイテムにはありません"));
									return builder;
								}
				...
				.then(new LiteralArgument("set")
					.then(new IntegerArgument("line", 1)
						.then(new GreedyStringArgument("name").executesPlayer((sender, args) -> {
							itemEdit(sender, builder -> builder.setLore((int) args[0] - 1, args[1].toString()));
						})))))
			.then(new LiteralArgument("grow")
				.executesPlayer((sender, args) -> {
					itemEdit(sender, builder -> builder.enchant(Enchantment.LURE, 0).flags(ItemFlag.HIDE_ENCHANTS));
				}))
			.register();
	}
