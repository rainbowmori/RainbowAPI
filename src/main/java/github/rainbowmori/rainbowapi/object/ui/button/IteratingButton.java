...
public class IteratingButton<T, MH extends MenuHolder<?>> extends ItemButton<MH> {
    ...
    /**
     * アイコンのみのIteratingButtonを作成します。サブクラスは {@link #currentState} と {@link #stateUpdater} フィールドを設定する必要があります。
     * または、{@link #getCurrentState()}と{@link #updateCurrentState(MenuHolder, InventoryClickEvent)}をオーバーライドする方法もある。
     * @param icon the icon
     */
     ...
     * {beforeToggle(MenuHolder, InventoryClickEvent)}と{@link #afterToggle(MenuHolder, InventoryClickEvent)}です。
    ...
    /**
     * アイコンの外観を決定します。
     * サブクラスは、{@link #setIcon(ItemStack)} を呼び出したりオーバーライドしたりせずに、このメソッドを安全にオーバーライドすることができます。
     * @param menuHolder the inventory holder for the menu
     * @param event the InventoryClickEvent that caused the button to toggle
