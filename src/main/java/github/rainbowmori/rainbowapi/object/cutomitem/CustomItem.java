...
/**
 * 左、右クリックやアイテムをもってシフトしたりアイテムをこれに変えたりほかのに変えたeventを受け取り処理するアイテムのabstract
 * classです サーバー起動時に登録する場合は {@link RMPlugin#registerItem(String,
 * Class)} で登録してください サーバーが起動中に登録する場合は {@link
 * #register(String, Class)} で登録してください
 * <p>
 * 引数を持たないコンストラクタを作成してそこで this(ItemStack)
 * でそのアイテムがデフォルトとして登録されます
 */
public abstract class CustomItem implements InterfaceItem {
  ...
  public CustomItem(ItemStack item) {
    if (!isRegister(getIdentifier())) {
      throw new RuntimeException(FormatterUtil.format(
          ...
          Map.of("name", getClass().getSimpleName(), "identifier",
              getIdentifier())));
    }
    NBT.modify(item, nbt -> {
  ...
  /**
   * カスタムアイテムの登録methodです、ですがサーバー起動時に登録する場合は
   * {@link RMPlugin#registerItem(String, Class)} を使用してください
   *
  ...
  public static void register(String identifier,
    ...
    if (isRegister(identifier)) {
      throw new IllegalArgumentException(
          identifier + "はすでにCustomItemに登録されています");
    }
    customItems.put(identifier, clazz);
    RainbowAPI.getPlugin().getPrefixUtil().logDebug(
        "<green>REGISTER CUSTOM ITEM [identifier=\"%s\",class=\"%s\"]"
  ...
  public static void unregister(String identifier) {
    if (!isRegister(identifier)) {
      throw new IllegalArgumentException(identifier +
          "はCustomItemに登録されていません");
    }
    RainbowAPI.getPlugin().getPrefixUtil().logDebug(
        "<red>UNREGISTER CUSTOM ITEM [identifier=\"%s\",class=\"%s\"]"
            .formatted(identifier, customItems.get(identifier).getName()));
  ...
  public static boolean isCustomItem(ItemStack item) {
    ...
    return NBT.get(item, readableNBT -> {
      ...
      return compound.hasTag("identifier") &&
          isRegister(compound.getString("identifier"));
    });
  }
   ...
   *                                  渡されたアイテムがCustomItemではない場合にthrowされます
   */
  @NotNull
  public static String getIdentifier(ItemStack item) {
    if (isCustomItem(item)) {
      return NBT.get(item, readableNBT -> {
        return readableNBT.getCompound(nbtKey).getString("identifier");
      });
    }
    throw new IllegalArgumentException(
        "そのアイテムはCustomItemではありません(もしくは登録されていません)");
  }
  ...
  public static CustomItem getCustomItem(ItemStack item) {
    if (isCustomItem(item)) {
      try {
        return customItems.get(getIdentifier(item))
   ...
   * @param consumer 処理
   */
  protected final void relativeBlock(PlayerInteractEvent e,
      Consumer<Block> consumer) {
    if (!clickedBlock(e)) {
      throw new RuntimeException(
          "clickedBlockがnullなのにrelativeBlockメゾットを呼びました");
    }
   ...
   * @param consumer 処理
   */
  protected final void relativeLocation(PlayerInteractEvent e,
